						...
						<table class="table table-bordered table-condensed table-hover">
							...
							<tbody>
								...
								if($_SESSION['login_type'] != 1): 
									$query = "SELECT r.*,p.plan,pp.package,concat(m.lastname,', ',m.firstname,' ',m.middlename) as name,m.member_id from registration_info r inner join members m on m.id = r.member_id inner join plans p on p.id = r.plan_id inner join packages pp on pp.id = r.package_id inner join subscribers sb on sb.member_id = m.member_id inner join users us on us.id = sb.user_id where r.status = 1 order by r.id asc";
								elseif($_SESSION['login_type'] == 1): 	
									$query = "SELECT r.*,p.plan,pp.package,concat(m.lastname,', ',m.firstname,' ',m.middlename) as name,m.member_id from registration_info r inner join members m on m.id = r.member_id inner join plans p on p.id = r.plan_id inner join packages pp on pp.id = r.package_id where r.status = 1 order by r.id asc";
								endif; 								
