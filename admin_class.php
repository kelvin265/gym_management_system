...
Class Action {
	...
	function login(){
			...
			if($qry->num_rows > 0){
				$status = "Active now";
                $this->db->query("UPDATE users SET status = '{$status}' WHERE username = '".$username."' and password = '".md5($password)."'");
	...
	function logout(){
		$status = "Offline now";
        $logout = $this->db->query("UPDATE users SET status = '{$status}' WHERE id={$_SESSION['login_id']}");
		if($logout){
			session_destroy();
			foreach ($_SESSION as $key => $value) {
				unset($_SESSION[$key]);
	...
	function assign_member(){
		...
		$chk = $this->db->query("Select * from subscribers where user_id = '$id' || member_id ='$member_id' ")->num_rows;
		if($chk > 0){
			return 2;
			exit;
		}
		if(empty($member_id)){
			...
			$save = $this->db->query("INSERT INTO subscribers set ".$data);
			if($save){
				return 1;
			}
		}
	...
	function get_schecdule(){
		...
		if($_SESSION['login_type'] != 1): 
			$query = "SELECT s.*,concat(m.lastname,',',m.firstname,' ', m.middlename) as name FROM schedules s inner join members m on m.id = s.member_id inner join subscribers sb on sb.member_id = m.member_id inner join users us on us.id = sb.user_id";
		elseif($_SESSION['login_type'] == 1): 	
			$query = "SELECT s.*,concat(m.lastname,',',m.firstname,' ', m.middlename) as name FROM schedules s inner join members m on m.id = s.member_id";
		endif; 
